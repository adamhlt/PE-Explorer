 ...
 */
const char* GetImageCharacteristics(const DWORD dCharacteristics)
{
 ...
 */
PIMAGE_SECTION_HEADER GetSections(const PIMAGE_SECTION_HEADER pImageSectionHeader, const int NumberOfSections, const DWORD dImportAddress)
{
 ...
 */
void GetImports32(PIMAGE_IMPORT_DESCRIPTOR pImageImportDescriptor, const DWORD dRawOffset, const PIMAGE_SECTION_HEADER pImageImportSection)
{
    printf("\n[+] IMPORTED DLL\n");
 ...
 * \param pImageImportSection : section wich contains imports.
 */
void GetImports64(PIMAGE_IMPORT_DESCRIPTOR pImageImportDescriptor, const DWORD dRawOffset, const PIMAGE_SECTION_HEADER pImageImportSection)
{
    printf("\n[+] IMPORTED DLL\n");
 ...
 */
PIMAGE_SECTION_HEADER GetExportSection(const PIMAGE_SECTION_HEADER pImageSectionHeader, const int NumberOfSections, const DWORD dExportAddress)
{
    PIMAGE_SECTION_HEADER pImageImportHeader = nullptr;
 ...
 */
void GetExports(const PIMAGE_EXPORT_DIRECTORY pImageExportDirectory, const DWORD dRawOffset, const PIMAGE_SECTION_HEADER pImageExportSection)
{
    printf("\n[+] EXPORTED FUNCTIONS\n\n");

    const DWORD dNumberOfNames = pImageExportDirectory->NumberOfNames;
...
{
    const auto pImageNTHeader32 = (PIMAGE_NT_HEADERS32)((DWORD)pImageDOSHeader + pImageDOSHeader->e_lfanew);
    ...
    const auto pImageSectionHeader = (PIMAGE_SECTION_HEADER)((DWORD)pImageNTHeader32 + 4 + sizeof(IMAGE_FILE_HEADER) + ImageFileHeader.SizeOfOptionalHeader);
    ...
    DWORD dRawOffset = (DWORD)pImageDOSHeader + pImageImportSection->PointerToRawData;
...
{
    const auto pImageNTHeader64 = (PIMAGE_NT_HEADERS64)((DWORD)pImageDOSHeader + pImageDOSHeader->e_lfanew);
    ...
    const auto pImageSectionHeader = (PIMAGE_SECTION_HEADER)((DWORD)pImageNTHeader64 + 4 + sizeof(IMAGE_FILE_HEADER) + ImageFileHeader.SizeOfOptionalHeader);
    if (pImageSectionHeader == nullptr)
        return -1;
...
int main(const int argc, char* argv[])
{
    ...
    const auto pImageNTHeader = (PIMAGE_NT_HEADERS)((DWORD)hFileContent + pImageDOSHeader->e_lfanew);
